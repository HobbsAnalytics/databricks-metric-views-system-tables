CREATE OR REPLACE VIEW mfg_mc_se_sa.sys_table_semantics.billing_usage_metrics
WITH METRICS
LANGUAGE YAML
AS $$
version: 1.1
...
# Point-in-time SCD joins use the scd_* helper views in this schema (raw dim +
# computed next_change_time). Product-scoped edges (cluster/job/pipeline) carry
# the billing_origin_product predicate inside the join `on` clause so the FK only
# attaches where it is valid. All joins validated N:1 (0 many-to-many).
...
joins:
  ...
  # cluster: point-in-time + scoped to classic compute products
  - name: cluster
    source: mfg_mc_se_sa.sys_table_semantics.scd_clusters
  ...
  # SQL warehouse: point-in-time
  - name: warehouse
    source: mfg_mc_se_sa.sys_table_semantics.scd_warehouses
  ...
  - name: pool
    source: mfg_mc_se_sa.sys_table_semantics.scd_instance_pools
  ...
  # job: point-in-time + scoped to JOBS product
  - name: job
    source: mfg_mc_se_sa.sys_table_semantics.scd_jobs
  ...
  # pipeline: point-in-time + scoped to DLT product
  - name: pipeline
    source: mfg_mc_se_sa.sys_table_semantics.scd_pipelines
  ...
  # serving endpoint / entity: point-in-time
  - name: served
    source: mfg_mc_se_sa.sys_table_semantics.scd_served_entities
...
dimensions:
  - name: Usage Date
    ...
    comment: Calendar date of the usage record.
  - name: Usage Month
    expr: DATE_TRUNC('MONTH', source.usage_date)
    comment: Month bucket of the usage record.
  - name: Workspace ID
  ...
  - name: Billing Origin Product
    ...
    comment: Product that originated the usage (JOBS, ALL_PURPOSE, DLT, MODEL_SERVING, ...).
  - name: Usage Unit
    expr: source.usage_unit
    comment: Unit the usage is measured in (e.g. DBU).
  - name: Cluster Name
...
measures:
  - name: Usage Records
    expr: COUNT(1)
    comment: Number of usage records in the slice.
  - name: Usage Quantity
    expr: SUM(source.usage_quantity)
    comment: Total units consumed (e.g. DBUs).
  - name: List Cost (USD)
